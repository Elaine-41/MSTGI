...
class HEDS : public AlgorithmInterface<dist_t> {
 public:
    ...
    // F: levelsSkip verification counters (GTI_SHG_VERBOSE_LEVELSKIP=1 to print)
    mutable std::atomic<long long> levelsSkip_call_count{0};
    mutable std::atomic<long long> levelsSkip_total_levels_skipped{0};
    ...
    bool use_full_dim_level_skip_ = false;  // GTI_SHG_FULL_DIM_LEVEL_SKIP: use full-dim distance for level-skipping (no compression)
    ...
    std::vector<uint32_t> resultsProcessing_epoch_;  // S1: epoch per slot, avoid per-query std::fill
    mutable uint32_t search_epoch_{0};              // S1: incremented each search
    ...
    HEDS(
        ...
        :   element_levels_(max_elements),
            allow_replace_deleted_(allow_replace_deleted),
            use_full_dim_level_skip_(use_full_dim_level_skip) {
    ...
    dist_t getDisByLevel(tableint data1, tableint data2, int level) const {
        if (use_full_dim_level_skip_) {
            return fstdistfunc_(getDataByInternalId(data1), getDataByInternalId(data2), dist_func_param_);
        }
    ...
    void clearShortcuts() {
        // DynamicPGMIndex has no clear(); destruct and reconstruct in place
        Shortcuts.~DynamicPGMIndex();
        new (&Shortcuts) pgm::DynamicPGMIndex<dist_t, int>();
        denseDensity.clear();
        shortcutsSize = 0;
    }

    void buildShortcuts(int &num_base, float sample_ratio = 1.0f){
        ...
        // E: sample 60-70% when sample_ratio < 1 (GTI_SHG_SHORTCUT_SAMPLE_RATIO=0.65)
        std::vector<int> indices;
        if (sample_ratio >= 1.0f) {
            for (int i = 0; i < num_base; i++) indices.push_back(i);
        } else {
            int n_sample = (int)(num_base * sample_ratio);
            n_sample = std::max(1, std::min(n_sample, num_base));
            std::vector<int> all(num_base);
            for (int i = 0; i < num_base; i++) all[i] = i;
            std::shuffle(all.begin(), all.end(), level_generator_);
            indices.assign(all.begin(), all.begin() + n_sample);
        }

        for (int idx : indices) {
            tableint cur_point = idx;
            int point_level = element_levels_[idx];
            while(point_level > 1){
                int skipLevel = 1;
                int skip_to_level = point_level -2;
    ...
    int levelsSkip(dist_t dis) const {
        ...
        if(it != Shortcuts.end()) skip = it->second;
        levelsSkip_call_count++;
        levelsSkip_total_levels_skipped += skip;
        return skip;
    }
    ...
    bool pruneDisCompute(dist_t currBound, int currLevel, tableint query_Id ,int currCand, std::vector<dist_t>& resultsProcessing) const{
        ...
        for(int l = maxlevel_; l > currLevel; l--){
            ...
            // S1: treat as invalid unless written in current search (epoch match)
            if(dis_level != (dist_t)-1 && start_pos + l < (int)resultsProcessing_epoch_.size() && resultsProcessing_epoch_[start_pos + l] == search_epoch_){
    ...
    searchKnnShortcuts(Query &query, BaseFilterFunctor* isIdAllowed = nullptr) {
        ...
        search_epoch_++;  // S1: invalidate previous query's cache without std::fill
        tableint currObj = enterpoint_node_;


        for(int level = maxlevel_; level > 0;){
            dist_t curdist = getDisByLevel(query.query_Id, currObj, level);
            size_t idx = currObj * (maxlevel_+1) + level;
            resultsProcessing[idx] = curdist;
            if (idx < resultsProcessing_epoch_.size()) resultsProcessing_epoch_[idx] = search_epoch_;
            ...
            while(changed){
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    size_t cand_idx = cand * (maxlevel_+1) + level;
                    resultsProcessing[cand_idx] = d;
                    if (cand_idx < resultsProcessing_epoch_.size()) resultsProcessing_epoch_[cand_idx] = search_epoch_;
            ...
            if(shortcutsSize < 100){
                ...
                int skip = levelsSkip(curdist);
                int skip_to_level = level - skip;
        ...
        // S4: GTI_SHG_PRUNE=0 uses searchBaseLayerST (no prune) for faster search, trade recall
        bool use_prune = true;
        if (const char *s = std::getenv("GTI_SHG_PRUNE"); s && s[0] == '0') use_prune = false;
...
//                    if (candidate_id == 0) continue;
    ...
    */
    ...
    /** S2 轻量级 query：仅覆盖 query 槽的 raw data 与 data_rep，不做图邻接更新。
     *  用于每查询 reuse 固定 query 槽时，替代 addDataPoint+markDelete，降低约 10–20% 搜索延迟。
     *  方向 1：use_full_dim_level_skip_ 时仅 memcpy raw，跳过 data_rep。
     *  方向 3：复用 thread_local rep buffer 减少分配。 */
    void overwriteQuerySlotData(const void *data_point, tableint internalId) {
        memcpy(getDataByInternalId(internalId), data_point, data_size_);
        if (use_full_dim_level_skip_) {
            return;  // 跳层用 raw，无需 data_rep
        }
        size_t rep_cap = data_rep_size_ + data_dim_;
        static thread_local std::vector<float> rep;
        if (rep.size() < rep_cap) rep.resize(rep_cap);
        rep.clear();
        rep.reserve(rep_cap);
        float *data_ptr = (float *)getDataByInternalId(internalId);
        for (size_t i = 0; i < data_dim_; i++) rep.push_back(data_ptr[i]);
        int previous_level_pos = 0;
        for (int cur_lev = 1; cur_lev <= (int)maxFixLevel_; cur_lev++) {
            int previous_level_size = getDatasizeLevel(cur_lev - 1);
            for (int i = 0; i < previous_level_size; i += (int)k_) {
                float sum = 0;
                if (i + (int)k_ > previous_level_size) {
                    for (int j = i; j < previous_level_size; j++) sum += rep[previous_level_pos + j];
                    rep.push_back(float(sum) / float(previous_level_size - i));
                } else {
                    for (int j = i; j < i + (int)k_; j++) sum += rep[previous_level_pos + j];
                    rep.push_back(float(sum) / float(k_));
                }
            }
            previous_level_pos += previous_level_size;
        }
        rep.erase(rep.begin(), rep.begin() + getDatasizeLevel(0));
        memcpy(data_rep_memory_ + internalId * data_rep_size_ * sizeof(float), rep.data(), rep.size() * sizeof(float));
    }
    ...
    */
    ...
    searchKnn(const void *query_data, tableint query_Id,  size_t k, BaseFilterFunctor* isIdAllowed = nullptr) const {
        ...
        dist_t curdist = getDisByLevel(query_Id, enterpoint_node_, maxFixLevel_);
        ...
        //for (int level = maxFixLevel_; level > 0; ) {
            ...
            while (changed) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    dist_t d = getDisByLevel(query_Id, cand, level);
