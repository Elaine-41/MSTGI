...
// ========== GTI format readers (fvecs / ivecs) ==========
// fvecs: each vector = 4 bytes dim + dim*4 bytes float
// ivecs: each row = 4 bytes k + k*4 bytes int

template <typename dist_t>
void readInitDataFvecs(int32_t &dim, int32_t &max_elements, dist_t*& data, string data_file_path) {
    cout << "Read data (fvecs format)" << endl;
    ...
    max_elements = (int32_t)(fsize / ((size_t)(dim + 1) * 4));
    if (max_elements > 10000000) max_elements = 10000000;
    data = new dist_t[(size_t)max_elements * dim];
    for (int32_t i = 0; i < max_elements; i++) {
        in.seekg(4, std::ios::cur);
        in.read((char*)(data + i * dim), dim * sizeof(dist_t));
    }
    in.close();
}

template <typename dist_t>
void readQuerysFvecs(int32_t &query_sum, int32_t &query_dim, dist_t*& querys, string query_file_path) {
    cout << "Read query (fvecs format)" << endl;
    std::ifstream in(query_file_path, std::ios::binary);
    if (!in.is_open()) {
        cout << "query_reader open failed!!!!" << endl;
        throw std::runtime_error("Cannot open query file");
    }
    in.read((char*)&query_dim, 4);
    in.seekg(0, std::ios::end);
    size_t fsize = in.tellg();
    in.seekg(0, std::ios::beg);
    query_sum = (int32_t)(fsize / ((size_t)(query_dim + 1) * 4));
    querys = new dist_t[(size_t)query_sum * query_dim];
    for (int32_t i = 0; i < query_sum; i++) {
        in.seekg(4, std::ios::cur);
        in.read((char*)(querys + i * query_dim), query_dim * sizeof(dist_t));
    }
    in.close();
}

// ivecs: each row = 4 bytes (k) + k*4 bytes (int). Output flattened matrix with groundtruth_dim = max k.
template <typename dist_t>
void readGroundTruthIvecs(int32_t &groundtruth_sum, int32_t &groundtruth_dim, uint32_t*& groundtruth,
                          string groundtruth_file_path, int K) {
    cout << "Read groundtruth (ivecs format)" << endl;
    std::ifstream in(groundtruth_file_path, std::ios::binary);
    if (!in.is_open()) {
    ...
    while (true) {
        ...
        if (in.eof()) break;
        if (num_elements > max_k) max_k = num_elements;
        std::vector<int> row(num_elements);
        in.read((char*)row.data(), num_elements * 4);
        rows.push_back(std::move(row));
    }
    in.close();
    groundtruth_sum = (int32_t)rows.size();
    groundtruth_dim = max_k;
    if (groundtruth_dim < K) {
        cout << "groundtruth_dim too small!!!!" << endl;
        throw std::runtime_error("groundtruth_dim < K");
    }
    groundtruth = new uint32_t[(size_t)groundtruth_sum * groundtruth_dim];
    for (int32_t i = 0; i < groundtruth_sum; i++) {
        size_t row_k = rows[i].size();
        for (size_t j = 0; j < row_k; j++)
            groundtruth[i * groundtruth_dim + j] = (uint32_t)rows[i][j];
        for (size_t j = row_k; j < (size_t)groundtruth_dim; j++)
            groundtruth[i * groundtruth_dim + j] = 0xFFFFFFFFu;
    }
}
