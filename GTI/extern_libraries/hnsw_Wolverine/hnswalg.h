...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
 public:
    ...
    HierarchicalNSW(
        ...
        linkLists_ = (char **) calloc(max_elements_, sizeof(void *));  // zero-init so level-0 slots stay nullptr
        if (linkLists_ == nullptr)
            throw std::runtime_error("Not enough memory: HierarchicalNSW failed to allocate linklists");
        size_links_per_element_ = maxM_ * sizeof(tableint) + sizeof(linklistsizeint);
        mult_ = 1 / log(1.0 * M_);
        revSize_ = 1.0 / mult_;
        deleteFlags = (bool *)malloc(max_elements_ * sizeof(bool));
        if (deleteFlags == nullptr)
            throw std::runtime_error("Not enough memory: HierarchicalNSW failed to allocate deleteFlags");
        memset(deleteFlags, 0, max_elements_ * sizeof(bool));
    }
...
//                    if (candidate_id == 0) continue;
    ...
    void resizeIndex(size_t new_max_elements) {
        ...
        // Reallocate deleteFlags to match new size
        bool *deleteFlags_new = (bool *)realloc(deleteFlags, new_max_elements * sizeof(bool));
        if (deleteFlags_new == nullptr)
            throw std::runtime_error("Not enough memory: resizeIndex failed to allocate deleteFlags");
        deleteFlags = deleteFlags_new;
        if (new_max_elements > element_levels_.size())
            memset(deleteFlags + cur_element_count, 0, (new_max_elements - cur_element_count) * sizeof(bool));
    ...
    void loadIndex(const std::string &location, SpaceInterface<dist_t> *s, size_t max_elements_i = 0) {
        ...
        // 加载索引时，若从文件构造器调用，成员可能未初始化，需先置空以防 clear() 出现未定义行为
        data_level0_memory_ = nullptr;
        linkLists_ = nullptr;
        cur_element_count = 0;
        deleteFlags = nullptr;
        ...
        deleteFlags = (bool *)malloc(max_elements_ * sizeof(bool));
        if (deleteFlags == nullptr)
            throw std::runtime_error("Not enough memory: loadIndex failed to allocate deleteFlags");
        memset(deleteFlags, 0, max_elements_ * sizeof(bool));
    ...
    */
    ...
    // Get neighbor labels at level 0 for a given label (for tree-augmented patch)
    std::vector<labeltype> getNeighborsForLabel(labeltype label, int level = 0) {
        std::vector<labeltype> out;
        std::unique_lock<std::mutex> lock_table(label_lookup_lock);
        auto it = label_lookup_.find(label);
        if (it == label_lookup_.end()) return out;
        tableint internalId = it->second;
        lock_table.unlock();
        std::vector<tableint> conn = getConnectionsWithLock(internalId, level);
        for (tableint cid : conn) {
            if (!deleteFlags[cid])
                out.push_back(getExternalLabel(cid));
        }
        return out;
    }
    ...
    void patchDelete(vector<labeltype>deleteList,int deleteModel,int newLinkSize,int num_threads){
        patchDelete(deleteList, deleteModel, newLinkSize, num_threads, nullptr);
    }

    // Overload with optional tree-augmented candidates and same-leaf priority (Direction 2)
    // tree_candidates_per_label: label -> [(dist_sq, neighbor_label), ...]
    // same_leaf_per_label: label -> set of labels in same tree leaf (for tie-breaking / priority)
    void patchDelete(vector<labeltype>deleteList,int deleteModel,int newLinkSize,int num_threads,
        const std::unordered_map<labeltype, std::vector<std::pair<dist_t, labeltype>>>* tree_candidates_per_label=nullptr,
        const std::unordered_map<labeltype, std::unordered_set<labeltype>>* same_leaf_per_label=nullptr){
        ...
        patchDeleteInternalDeleteList(internalDeleteList,deleteModel,num_threads,newLinkSize,changeEp,tree_candidates_per_label,same_leaf_per_label);
    }
    ...
    // Same-leaf boost: prefer candidates in same tree leaf (Direction 2, ~0.1% dist reduction)
    static constexpr dist_t SAME_LEAF_BOOST = 0.999;
    inline dist_t applySameLeafBoost(dist_t d, labeltype thePoint_label, tableint predict,
        const std::unordered_map<labeltype, std::unordered_set<labeltype>>* same_leaf_per_label) const {
        if (!same_leaf_per_label) return d;
        auto it = same_leaf_per_label->find(thePoint_label);
        if (it == same_leaf_per_label->end()) return d;
        if (it->second.count(getExternalLabel(predict))) return d * SAME_LEAF_BOOST;
        return d;
    }
    ...
    inline void patchDeleteInternalDeleteList(vector<tableint> internalDeleteList,int deleteModel,int num_threads,int newLinkSize,bool changeEp,
        const std::unordered_map<labeltype, std::vector<std::pair<dist_t, labeltype>>>* tree_candidates_per_label=nullptr,
        const std::unordered_map<labeltype, std::unordered_set<labeltype>>* same_leaf_per_label=nullptr){
        dist_t same_leaf_boost = (dist_t)0.999;
        if (const char* s = std::getenv("GTI_SAME_LEAF_BOOST")) {
            same_leaf_boost = (dist_t)std::strtod(s, nullptr);
            if (same_leaf_boost <= 0 || same_leaf_boost > 1) same_leaf_boost = (dist_t)0.999;
        }
        ...
        ParallelFor(0, cur_element_count, num_threads, [&](size_t row, size_t threadId) {
            for(int level=element_levels_[row];level>=0;level--){   //update inpoint  
                ...
                if(deleteModel==PINTOPOUT_DELETE && !deleteFlags[row]){
                    ...
                    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates;
                    labeltype row_label=getExternalLabel(row);
                    const std::unordered_set<labeltype>* row_same_leaf = nullptr;
                    if(same_leaf_per_label){ auto it=same_leaf_per_label->find(row_label); if(it!=same_leaf_per_label->end()) row_same_leaf=&it->second; }
                    for(tableint cand : cand_list){
                        dist_t d=fstdistfunc_(getDataByInternalId(cand), getDataByInternalId(row), dist_func_param_);
                        if(row_same_leaf && row_same_leaf->count(getExternalLabel(cand))) d*=same_leaf_boost;
                        top_candidates.emplace(make_pair(d,cand));
                    }
                ...
                for(int level=element_levels_[internalId];level>=0;level--){
                    ...
                    if(deleteModel==SEARCH_DELETE){
                        for(int linkID=0;linkID<internalId_size;linkID++){
                            ...
                            std::vector<std::pair<dist_t, tableint>> cand_vec;
                            while (!top_candidates.empty()) {
                                cand_vec.push_back(top_candidates.top());
                                top_candidates.pop();
                            }
                            if (tree_candidates_per_label) {
                                labeltype thePoint_label = getExternalLabel(thePoint);
                                auto it = tree_candidates_per_label->find(thePoint_label);
                                if (it != tree_candidates_per_label->end()) {
                                    std::unordered_set<tableint> cand_ids(cand_vec.size());
                                    for (const auto& p : cand_vec) cand_ids.insert(p.second);
                                    std::unique_lock<std::mutex> lock_table(label_lookup_lock);
                                    for (const auto& p : it->second) {
                                        labeltype cand_label = p.second;
                                        auto lit = label_lookup_.find(cand_label);
                                        if (lit != label_lookup_.end() && !deleteFlags[lit->second]) {
                                            tableint cand_internal = lit->second;
                                            if (cand_ids.find(cand_internal) == cand_ids.end()) {
                                                dist_t d = (dist_t)p.first;
                                                cand_vec.push_back({d, cand_internal});
                                                cand_ids.insert(cand_internal);
                                                if (cand_vec.size() > 8 * (size_t)newLinkSize) break;
                                            }
                                        }
                                    }
                                }
                            }
                            labeltype thePoint_label_sl = getExternalLabel(thePoint);
                            const std::unordered_set<labeltype>* sl_set = nullptr;
                            if (same_leaf_per_label) {
                                auto it = same_leaf_per_label->find(thePoint_label_sl);
                                if (it != same_leaf_per_label->end()) sl_set = &it->second;
                            }
                            for (auto& p : cand_vec) {
                                if (sl_set && sl_set->count(getExternalLabel(p.second)))
                                    p.first *= same_leaf_boost;
                            }
                            std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> merged_pq;
                            for (const auto& p : cand_vec)
                                merged_pq.emplace(p.first, p.second);
                            getNeighborsByHeuristic2(merged_pq, newLinkSize);
                            addCandToNewLink(newLink, merged_pq, level, thePoint);
                        }
                    }
                        ...
                        for(int linkID=0;linkID<internalId_size;linkID++){
                            ...
                            std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates;
                            labeltype thePoint_label_tw=getExternalLabel(thePoint);
                            const std::unordered_set<labeltype>* tw_same_leaf=nullptr;
                            if(same_leaf_per_label){ auto it=same_leaf_per_label->find(thePoint_label_tw); if(it!=same_leaf_per_label->end()) tw_same_leaf=&it->second; }
                            for(tableint predict:predict_list){
                                dist_t d=fstdistfunc_(getDataByInternalId(thePoint), getDataByInternalId(predict), dist_func_param_);
                                if(tw_same_leaf && tw_same_leaf->count(getExternalLabel(predict))) d*=same_leaf_boost;
                                top_candidates.emplace(make_pair(d,predict));
                            }
                        ...
                        for(int linkID=0;linkID<internalId_size;linkID++){
                            ...
                            std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates;
                            unordered_set<tableint> predict_list;
                            // Tree-augmented: inject candidates from GTI searchTreeRange
                            if(tree_candidates_per_label){
                                labeltype thePoint_label=getExternalLabel(thePoint);
                                auto it=tree_candidates_per_label->find(thePoint_label);
                                if(it!=tree_candidates_per_label->end()){
                                    ...
                                    for(const auto& p:it->second){
                                        ...
                                        auto lit=label_lookup_.find(cand_label);
                                        if(lit!=label_lookup_.end()&&!deleteFlags[lit->second]){
                                            predict_list.emplace(lit->second);
                                            if(predict_list.size()>8*newLinkSize)break;
                                        }
                                    }
                                }
                            }
                            ...
                            labeltype thePoint_label_approx=getExternalLabel(thePoint);
                            const std::unordered_set<labeltype>* approx_same_leaf=nullptr;
                            if(same_leaf_per_label){ auto it=same_leaf_per_label->find(thePoint_label_approx); if(it!=same_leaf_per_label->end()) approx_same_leaf=&it->second; }
                            for(tableint predict:predict_list){
                                dist_t d=fstdistfunc_(getDataByInternalId(thePoint), getDataByInternalId(predict), dist_func_param_);
                                if(approx_same_leaf && approx_same_leaf->count(getExternalLabel(predict))) d*=same_leaf_boost;
                                top_candidates.emplace(make_pair(d,predict));
                            }
                            getNeighborsByHeuristic2(top_candidates, newLinkSize);                   
